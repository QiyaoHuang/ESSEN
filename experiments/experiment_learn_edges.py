...
np.seterr(all='raise') 
parser = argparse.ArgumentParser('Interface for ESSEN experiments on link predictions')
parser.add_argument('-g', type=str, default='0', help='idx for the gpu to use')
parser.add_argument('-b', type=int, default='512', help='num for the batch to use')
parser.add_argument('-s', type=str, default='w', help='setting json')
...
if 'data_name' in parameter_dict:
    h_learn_edge.ex.observers.append(FileStorageObserver('./results/ESSEN/'+parameter_dict['data_name']))
else:
    h_learn_edge.ex.observers.append(FileStorageObserver('./results/ESSEN/'+default_configuration.PARAMETER_DICT['data_name']))
